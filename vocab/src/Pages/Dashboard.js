...
function Dashboard() {
  ...
  const handleModuleClick = (level, moduleIndex) => {
    navigate(`/hello/${level + 1}/${moduleIndex + 1}`);
  };
  ...
  const handleOptionClick = async (option) => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:5000/updateStreak/${userId}`,
        {
        ...
        >
          <Box
            sx={{
              ...
              msOverflowStyle: "none", // Correct camelCase for Internet Explorer and Edge
              scrollbarWidth: "none",
              ...
              <Box key={index}>
                <Button
                  ...
                  sx={{
                    ...
                    color: "#000",
                    padding: "10px 20px",
                    borderRadius: expandedLevel === index ? "20px" : "120px",
                    fontWeight: "bold",
                    boxShadow: expandedLevel === index
                      ? "none"
                      : "13px 13px 44px #462702, inset 30px 0px 16px rgba(255, 248, 248, 0.25), inset -20px -20px 40px #B5AC9E",
                    background: "#F6F0F0",
                ...
                {expandedLevel === index && (
                  ...
                  >
                    {(progressDataByLevel[level] || minimumModules).map(
                      (progress, idx) => (
                        <Box
                          onClick={() => handleModuleClick(index, idx)}
                          key={idx}
                          sx={{
                            display: "flex",
                            alignItems: "center",
                        ...
                        backgroundColor:
                          selectedOption === option
                            ? option === correctAnswer
                              ? "#A4DE8C" // Green for correct
                              : "#FFB5B5" // Red for incorrect
                            : "#FFE9C7", // Default option color
                        padding: "10px",
                        borderRadius: "5px",
                        cursor: "pointer",
                        "&:hover": {
