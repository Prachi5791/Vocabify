...
const Quiz = () => {
    ...
    useEffect(() => {
        const getQuestions = async () => {
            try {
                const fetchedQuestions = await fetchQuestions(level, module);
    ...
    const handleAnswerSelect = (option) => {
        console.log("Selected Answer:", option.option);
    ...
    const handleClosePopup = () => {
        ...
        setTimeout(() => {
            ...
            // Reset the selected answer, result, and any other state if necessary
            setSelectedAnswer("");
            setResult("");
            setShowHint(false); // Reset hint if shown
        }, 500); // Small delay to allow the popup to close smoothly
    };
                  ...
                  {currentQuestion.options.map((option, index) => {
                    ...
                    return (
                      <Box
                        key={index}
                        sx={{
                          flex: "1 1 calc(50% - 10px)", // Adjust for 2x2 grid
                          display: "flex",
                          flexDirection: "column",
                          alignItems: "center",
                          gap: 2,
