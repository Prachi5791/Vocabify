...
router.get('/dailyQuestion', async (req, res) => {
  try {
    ...
    const question = await DailyQuestion.findOne({
      date: { $lte: today },
      expiresAt: { $gte: today }
    }).sort({ date: -1 }); // Get the most recent question within date range

    if (!question) {
      return res.status(404).json({ message: 'No question available for today' });
    }

    // Reformat the question for the frontend
    const questionData = {
    ...
    res.status(200).json(questionData); // Send the question and options as JSON
  } catch (error) {
    console.error('Error fetching daily question:', error);
    res.status(500).json({ message: 'Server error' });
  }
