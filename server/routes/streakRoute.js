...
// Update streakAttempted status
router.post('/updateStreakAttempted/:userId', async (req, res) => {
  try {
    const { userId } = req.params;
    const { isAttempted } = req.body; // Extract from request body
...
// Update user streak
// Assuming you have a `userModel` and the route is correctly set up
router.post('/updateStreak/:userId', async (req, res) => {
  try {
    const { userId } = req.params;  // Extract userId from the URL
    const { isCorrect } = req.body; // Get whether the answer was correct from the request body
